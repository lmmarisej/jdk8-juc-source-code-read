...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
    ...
    // CompletableFuture用这个存储后续依赖于它的任务
    volatile Completion stack;    // Top of Treiber stack of dependent actions  无锁并发栈，栈顶指针
    ...
    static final class UniApply<T, V> extends UniCompletion<T, V> {
        ...
        UniApply(Executor executor,             // 执行自己的ForkJoinPool
                 CompletableFuture<V> dep,      // 输出一个CompletableFuture，thenApply的返回值
                 CompletableFuture<T> src,      // 其依赖的前置任务
                 Function<? super T, ? extends V> fn        // 要执行的任务逻辑
        ) {
    ...
    private CompletableFuture<Void> uniAcceptStage(Executor e,
        ...
        if (e != null       // thenApplyAsync
                || !d.uniAccept(this, f, null)      // thenApply——前置任务已完成：直接调用处理；否则，入栈（这种情况下和thenApplyAsync没有区别）
        ) {
            UniAccept<T> c = new UniAccept<T>(e, d, this, f);       // thenApplyAsync使用UniAccept构建对象入栈
            push(c);
            c.tryFire(SYNC);
        }
        return d;
    ...
    @SuppressWarnings("serial")
    // 中转任务，本身没有任务代码，只是参照输入的两个future是否完成，完成就从栈中弹出依赖他的BiRelay任务，然后执行。
    ...
    static final class AsyncSupply<T> extends ForkJoinTask<Void>
        ...
        public void run() {
            ...
            if ((d = dep) != null && (f = fn) != null) {
                dep = null;
                fn = null;
                if (d.result == null) {     // 任务未完成
                    try {
                        d.completeValue(f.get());       // 将Supplier#get的执行结果塞入CompletableFuture
                    } catch (Throwable ex) {
                        d.completeThrowable(ex);
                    }
                }
                d.postComplete();       // 将第二步的任务弹出执行
            }
        }
    }
    ...
    {
        if (f == null) throw new NullPointerException();
        CompletableFuture<U> d = new CompletableFuture<U>();        // 任务返回值返回值
